...
#include "no5tlbase.h"	// CPointerArray
#include "utils.h"		// CDCSaver
...
public:
	DECLARE_WND_CLASS_EX(_T("MarqueeWndCls"), CS_VREDRAW | CS_HREDRAW | CS_DBLCLKS, COLOR_WINDOW)
	...
	LRESULT OnRButtonDown(UINT /*uMsg*/, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
	{
		...
		if(GetElapse() <= 1 )
			...
			flags = def;
		menu.AppendMenu(flags,ID_MARQUEE_FASTER,_T("Faster"));
		
		if(GetElapse() >=1024)
			flags = def | MF_GRAYED;
		else
			flags = def;
		menu.AppendMenu(flags,ID_MARQUEE_SLOWER, _T("Slower"));
		
		menu.AppendMenu(MF_ENABLED|MF_SEPARATOR,-1);
		menu.AppendMenu(MF_ENABLED|MF_STRING,ID_MARQUEE_BKCLR, _T("Background color"));
		menu.AppendMenu(MF_ENABLED|MF_STRING,ID_MARQUEE_TXTCLR, _T("Font color"));
		//menu.AppendMenu(MF_ENABLED|MF_STRING,ID_MARQUEE_FONT,"Font");
		menu.AppendMenu(MF_ENABLED|MF_SEPARATOR,-1);
		menu.AppendMenu(MF_ENABLED|MF_STRING,ID_MARQUEE_EMPTY, _T("Empty"));
		menu.AppendMenu(MF_ENABLED|MF_STRING,ID_MARQUEE_PAUSE,IsPaused() ? _T("continue") : _T("pause"));
		menu.AppendMenu(MF_ENABLED|MF_SEPARATOR,-1);
		menu.AppendMenu(MF_ENABLED|MF_STRING,ID_MARQUEE_DEF, _T("Default values"));
		GetCursorPos(&pt);
...
public:
	...
	{
		return AddItem(s, m_clrTxt);
	}
	BOOL AddItem(LPCTSTR s,COLORREF clr)
	{
		ATLASSERT(IsWindow());
	...
	CMarqueeWnd(const CMarqueeWnd& wnd):m_bToLeft(true)
	{
		if (this != &wnd) {
			Copy(wnd);
		}
	}
	CMarqueeWnd& operator = (const CMarqueeWnd& wnd)
	{
		return Copy(wnd);
	}
	CMarqueeWnd& Copy(const CMarqueeWnd& wnd)
	{
		m_timer = wnd.m_timer;
		m_nElapse = wnd.m_nElapse;
		m_font = wnd.m_font;
		m_clrBack = wnd.m_clrBack;
		m_clrTxt = wnd.m_clrTxt;
		m_bLoop = wnd.m_bLoop;
		m_bpi = wnd.m_bpi;
		m_space = wnd.m_space;
		for (int i = 0; i < wnd.m_items.GetSize(); i++) {
			m_items.Add(wnd.m_items[i]);
		}
		m_hWnd = wnd.m_hWnd;
		return *this;
	}
