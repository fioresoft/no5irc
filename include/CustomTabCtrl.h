...
// Revised version by Daniel Bowen (bowen.daniel@gmail.com).
// Copyright (c) 2001-2002 Bjarke Viksoe.
// Copyright (c) 2002-2007 Daniel Bowen.
//
...
// ----------------------------------
//
// 2007/06/06: Daniel Bowen
// - Add full support for "multi-line" style tabs, enabled with the style bit
//   CTCS_MULTILINE (and the optional CTCS_RAGGEDRIGHT).
//   (this was a sponsored enhancement)
// - CTCS_MULTILINE is meant to work similar to the multi-line style of the
//   common control tabs (SysTabCtrl32). Adding support for multi-line tabs
//   has caused a few things to change in a non-backward compatible way.
//   The biggest change is that the size needs to be up to the tab control,
//   not the container (if its to be shown). This is communicated with
//   CalculateBounds, the notification CTCN_NEEDRESIZE, a refactored
//   UpdateLayout, and so on.  The functions involved with implementing 
//   multi-line style tabs includes the overrideable CalcRowCount_MultiLine
//   and UpdateLayout_MultiLine.  UpdateLayout_MultiLine calls many helper
//   functions that are all overrideable. Note that the close button is
//   supported with multiline tabs (but scroll buttons aren't).
// - Add new notification CTCN_NEEDRESIZE, which uses new NMCTCSIZE.
// - Add new settings variables in CTCSETTINGS:
//    Tab item related: iTabDefaultHeight, iTabForcedHeight, iTabForcedWidth,
//     iTabMinWidth
//    Tab area related settings: iTabAreaPaddingLeft, iTabAreaPaddingRight,
//     iTabAreaPaddingTop, iTabAreaPaddingBottom
// - Obsolete CalcSize_NonClient.  Instead, expand the definition of
//   CTCSETTINGS to include border and padding, and use that. We need that in
//   part because of how multi-line tabs can grow or shrink.
// - Add or enhance the following methods:
//    GetRowCount, CalculateBounds, AdjustRect (that works like TCM_ADJUSTRECT)
//    GetItemSize, SetItemSize (now works like TCM_SETITEMSIZE),
//    GetMinTabWidth, SetMinTabWidth (works like TCM_SETMINTABWIDTH),
//    CalcSize_Item_FullSize.
// - Have 2 UpdateLayout calls.  The one without a RECT will first call
//   CalculateBounds, and make sure that the bounds equals the size of the tab
//   rect. If there's a difference, the CTCN_NEEDRESIZE notification is sent
//   to the parent. The other UpdateLayout override takes the clientRect,
//   and uses the sizes and row count already calculated to lay things out.
// - Change OnSize implementation to call new UpdateLayout that takes
//   RECT (so it can skip default UpdateLayout that now calls the new
//   CalculateBounds to make sure things are sized right).
// - Define CTCS_VERTICAL, and use it in a few places.  Its still a ways off
//   from being fully supported.
// - Add SetRect overload on CCustomTabItem.
// - Add "ContentOffsetLeft" and "ContentOffsetRight" to CCustomTabItem used
//   for optionally centering the image and text (used when a multi-line
//   tab goes to muliple lines).
//
...
//#define CTCS_OLEDRAGDROP         0x1000   // TCS_FOCUSONBUTTONDOWN
...
#define CTCN_NEEDRESIZE         (TCN_FIRST - 20)
...
typedef struct tagNMCTCSIZE
{
	NMHDR   hdr;
	RECT    rect;
} NMCTCSIZE, *LPNMCTCSIZE;
...
typedef struct tagCTCSETTINGS
{
	...
	signed char iTabDefaultHeight;
	signed char iTabForcedHeight;
	signed char iTabForcedWidth;
	signed char iTabMinWidth;

	// Tab area related settings
	signed char iTabAreaPaddingLeft;
	signed char iTabAreaPaddingRight;
	signed char iTabAreaPaddingTop;
	signed char iTabAreaPaddingBottom;
...
protected:
	...
	bool m_reserved1;
	bool m_reserved2;
	short m_nContentOffsetLeft;
	short m_nContentOffsetRight;
...
public:
	CCustomTabItem() :
		...
		m_reserved1(false),
		m_reserved2(false),
		m_nContentOffsetLeft(0),
		m_nContentOffsetRight(0)
	{
	...
	const CCustomTabItem& operator=(const CCustomTabItem& rhs)
	{
		if(&rhs != this)
		{
			...
			m_nContentOffsetLeft = rhs.m_nContentOffsetLeft;
			m_nContentOffsetRight = rhs.m_nContentOffsetRight;
		}
		return *this;
	}
...
public:
	...
	{
		::SetRect(&m_rcItem, left, top, right, bottom);
		return true;
	}
	...
	short GetContentOffsetLeft() const
	{
		return m_nContentOffsetLeft;
	}
	bool SetContentOffsetLeft(short offsetLeft)
	{
		m_nContentOffsetLeft = (offsetLeft > 0) ? offsetLeft : (short)0;
		return true;
	}
	short GetContentOffsetRight() const
	{
		return m_nContentOffsetRight;
	}
	bool SetContentOffsetRight(short offsetRight)
	{
		m_nContentOffsetRight = (offsetRight > 0) ? offsetRight : (short)0;
		return true;
	}
	bool SetContentOffset(short offsetLeft, short offsetRight)
	{
		m_nContentOffsetLeft = (offsetLeft > 0) ? offsetLeft : (short)0;
		m_nContentOffsetRight = (offsetRight > 0) ? offsetRight : (short)0;
		return true;
	}
	bool ClearContentOffset()
	{
		m_nContentOffsetLeft = (short)0;
		m_nContentOffsetRight = (short)0;
		return true;
	}
...
public:
	...
	//Real CAtlArray:  size_t Add( INARGTYPE element );
	size_t Add(E& element)
	{
		baseClass::Add(element);
		return (m_nSize-1);
	}
	...
	// Real CAtlArray: void RemoveAll() throw();
	void RemoveAll()
	{
		baseClass::RemoveAll();
	}
...
protected:
	...
	ATL::CAtlArray< short > m_RowOrder;
	WTL::CFont m_font;
	WTL::CFont m_fontSel;
...
public:
	...
	LRESULT OnSize(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& bHandled)
	{
		...
		m_RowOrder.RemoveAll();

		// NOTE: We'll assume that if we're getting a size message,
		//  its due to the UpdateLayout / WM_SIZE handling of our
		//  parent or owner, and that our bounds have already
		//  been calculated.  For other uses of UpdateLayout
		//  without parameters, we'll be doing a CalculateBounds
		//  first to make sure we're sized appropriately
		//  (in case, for example, the text of a tab has changed,
		//  and would require a new row in the multline tab).
		RECT clientRect={0};
		this->GetClientRect(&clientRect);

		pT->UpdateLayout(clientRect);
		this->Invalidate();
		bHandled = FALSE;
		return 0;
	}
	...
	LRESULT OnStyleChanged(UINT /*uMsg*/, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
	{
		if(wParam == GWL_STYLE)
		{
			...
			if(pStyles != NULL && (pStyles->styleOld != pStyles->styleNew))
			{
				T* pT = static_cast<T*>(this);
				...
				// Check for a change of style that would trigger different settngs
				if( (pStyles->styleOld & (CTCS_BOTTOM|CTCS_MULTILINE)) !=
					(pStyles->styleNew & (CTCS_BOTTOM|CTCS_MULTILINE)))
				{
					this->SendMessage(WM_SETTINGCHANGE, 0, 0L);
				}
...
public:
	...
	// Pass in the proposed rectangle for the tab area, and
	// get back a rectangle adjusted for what the tab control needs
	// (needed for multiline tabs)
	void CalculateBounds(RECT containerClientRect, LPRECT tabWindowRect, LPRECT viewWindowRect)
	{
		T* pT = static_cast<T*>(this);
		DWORD dwStyle = this->GetStyle();

		if(tabWindowRect != NULL)
		{
			::CopyRect(tabWindowRect, &containerClientRect);
		}
		if(viewWindowRect != NULL)
		{
			::CopyRect(viewWindowRect, &containerClientRect);
		}

		int tabHeight = m_settings.iTabDefaultHeight;
		if(m_settings.iTabForcedHeight > 0)
		{
			tabHeight = m_settings.iTabForcedHeight;
		}

		if(CTCS_MULTILINE == (dwStyle & CTCS_MULTILINE))
		{
			int constrainedSideLength = 0;
			if(CTCS_VERTICAL == (dwStyle & CTCS_VERTICAL))
			{
				constrainedSideLength = (containerClientRect.bottom - containerClientRect.top);
			}
			else
			{
				constrainedSideLength = (containerClientRect.right - containerClientRect.left);
			}
			...
			if(CTCS_CLOSEBUTTON == (dwStyle & CTCS_CLOSEBUTTON))
			{
				if( (m_iCurSel >= 0) && ((size_t)m_iCurSel < m_Items.GetCount()) )
				{
					...
					{
						RECT containerClientRectCopy={0}, rcCloseButtonCopy={0};
						::CopyRect(&containerClientRectCopy, &containerClientRect);
						::CopyRect(&rcCloseButtonCopy, &m_rcCloseButton);
						pT->CalcSize_CloseButton(&containerClientRectCopy);
						if(CTCS_VERTICAL == (dwStyle & CTCS_VERTICAL))
						{
							constrainedSideLength -= (containerClientRect.top - containerClientRectCopy.top);
						}
						else
						{
							constrainedSideLength -= (containerClientRect.right - containerClientRectCopy.right);
						}
						// Leave m_rcCloseButton as it was.  We'll let
						// UpdateLayout set the real size
						::CopyRect(&m_rcCloseButton, &rcCloseButtonCopy);
					}
				}
			}

			pT->CalcRowCount_MultiLine(constrainedSideLength);
		}
		else
		{
			m_iRowCount = 1;
			if(m_RowOrder.GetCount() != 1)
			{
				m_RowOrder.RemoveAll();
				short row=0;
				m_RowOrder.Add(row);
		...
		int tabUnconstrainedSideLength = (tabHeight * m_iRowCount);
		if(CTCS_VERTICAL == (dwStyle & CTCS_VERTICAL))
		{
			tabUnconstrainedSideLength += (m_settings.iTabAreaPaddingLeft + m_settings.iTabAreaPaddingRight);
			if(CTCS_BOTTOM == (dwStyle & CTCS_BOTTOM))
			{
				if(tabWindowRect != NULL)
				{
					tabWindowRect->left = containerClientRect.right - tabUnconstrainedSideLength;
				}
				if(viewWindowRect != NULL)
				{
					viewWindowRect->right = containerClientRect.right - tabUnconstrainedSideLength;
				}
			}
			else
			{
				if(tabWindowRect != NULL)
				{
					tabWindowRect->right = containerClientRect.left + tabUnconstrainedSideLength;
				}
				if(viewWindowRect != NULL)
				{
					viewWindowRect->left = containerClientRect.left + tabUnconstrainedSideLength;
				}
			}
		}
		else
		{
			tabUnconstrainedSideLength += (m_settings.iTabAreaPaddingTop + m_settings.iTabAreaPaddingBottom);
			if(CTCS_BOTTOM == (dwStyle & CTCS_BOTTOM))
			{
				if(tabWindowRect != NULL)
				{
					tabWindowRect->top = containerClientRect.bottom - tabUnconstrainedSideLength;
				}
				if(viewWindowRect != NULL)
				{
					viewWindowRect->bottom = containerClientRect.bottom - tabUnconstrainedSideLength;
				}
			}
			else
			{
				if(tabWindowRect != NULL)
				{
					tabWindowRect->bottom = containerClientRect.top + tabUnconstrainedSideLength;
				}
				if(viewWindowRect != NULL)
				{
					viewWindowRect->top = containerClientRect.top + tabUnconstrainedSideLength;
				}
			}
		}
	}

	// Method compatible with TCM_ADJUSTRECT.
	void AdjustRect(BOOL fLarger, LPRECT rect)
	{
		...
		DWORD dwStyle = this->GetStyle();
		LONG *topLeft=NULL, *bottomRight=NULL;
		int topLeftPadding=0, bottomRightPadding=0;
		if(CTCS_VERTICAL == (dwStyle & CTCS_VERTICAL))
		{
			topLeft = &(rect->left);
			bottomRight = &(rect->right);
			topLeftPadding = m_settings.iTabAreaPaddingLeft;
			bottomRightPadding = m_settings.iTabAreaPaddingRight;
		}
		else
		{
			topLeft = &(rect->top);
			bottomRight = &(rect->bottom);
			topLeftPadding = m_settings.iTabAreaPaddingTop;
			bottomRightPadding = m_settings.iTabAreaPaddingBottom;
		}

		if(fLarger)
		{
			// Go from display rectangle to what the full window bounds would need to be
			if(CTCS_BOTTOM == (dwStyle & CTCS_BOTTOM))
			{
				*bottomRight += tabHeight * m_iRowCount;
				*bottomRight += topLeftPadding + bottomRightPadding;
			}
			else
			{
				*topLeft -= tabHeight * m_iRowCount;
				*topLeft -= topLeftPadding + bottomRightPadding;
			}
		}
		else
		{
			// Go from window rectangle to view rectangle.
			if(CTCS_BOTTOM == (dwStyle & CTCS_BOTTOM))
			{
				*bottomRight -= tabHeight * m_iRowCount;
				*bottomRight -= topLeftPadding + bottomRightPadding;
	...
	void UpdateLayout(void)
	{
		...
		pT->CalculateBounds(clientRect, &tabWindowRect, NULL);

		// Check if the tabWindowRect needs to be bigger than it currently is
		if(!::EqualRect(&tabWindowRect, &clientRect))
		{
			// Tell our parent that we need to be resized.
			m_RowOrder.RemoveAll();

			NMCTCSIZE nmh = {
				{ m_hWnd, this->GetDlgCtrlID(), CTCN_NEEDRESIZE },
				{ tabWindowRect.left, tabWindowRect.top, tabWindowRect.right, tabWindowRect.bottom } };
			LRESULT result = ::SendMessage(GetParent(), WM_NOTIFY, nmh.hdr.idFrom, (LPARAM)&nmh);

			// NOTE: If you hit this assert, be sure that you are handling CTCN_NEEDRESIZE.
			// The tab control is now in charge of its size, but needs the help of its container
			// if it isn't already sized to what it needs.
			ATLASSERT(result == 1 && "The tab control container did not handle CTCN_NEEDRESIZE correctly");
		}
		else
		{
			pT->UpdateLayout(tabWindowRect);
		}
	}

	// The input rectangle should be the size of the window.
	// The output rectangle will be the resulting size the tab needs.
	// If sizing a view window based on the result, note that the
	// resulting tab rectangle can extend beyond the size of the window.
	void UpdateLayout(RECT tabWindowRect)
	{
		if(	!m_hWnd ||
			!::IsWindow(m_hWnd) ||
			(ectcEnableRedraw != (m_dwState & ectcEnableRedraw)))
		...
		if( (tabWindowRect.right - tabWindowRect.left) <= 0 ||
			(tabWindowRect.bottom - tabWindowRect.top) <= 0)
		{
			return;
		}
		...
		::CopyRect(&m_rcTabItemArea, &tabWindowRect);

		m_rcTabItemArea.left += m_settings.iTabAreaPaddingLeft;
		m_rcTabItemArea.right -= m_settings.iTabAreaPaddingRight;
		m_rcTabItemArea.top += m_settings.iTabAreaPaddingTop;
		m_rcTabItemArea.bottom -= m_settings.iTabAreaPaddingBottom;

		// We'll obsolete CalcSize_NonClient for now, and use some of the settings
		// to track the padding along the sides.
		...
		if(CTCS_MULTILINE == (dwStyle & CTCS_MULTILINE))
		{
			pT->UpdateLayout_MultiLine(m_rcTabItemArea);
		}
		else if(CTCS_SCROLL == (dwStyle & CTCS_SCROLL))
		{
	...
	void CalcSize_Item_FullSize(size_t index, CDCHandle dc, int* itemWidth)
	{
		if(itemWidth == NULL || index < 0 || index >= m_Items.GetCount())
		{
			return;
		}

		if(m_settings.iTabForcedWidth > 0)
		{
			*itemWidth = m_settings.iTabForcedWidth;
			return;
		}
		...
		*itemWidth = 0;
		...
		TItem* pItem = m_Items[index];
		ATLASSERT(pItem != NULL);

		if((int)index == m_iCurSel && !m_fontSel.IsNull())
		{
		...
		if(pItem->UsingText())
		{
			...
			width += (rcText.right - rcText.left) + (m_settings.iPadding * 2);
		}
		...
		if(m_settings.iTabMinWidth > 0 && width < m_settings.iTabMinWidth)
		{
			width = m_settings.iTabMinWidth;
		}

		*itemWidth = width;
	}

	int CalcRowCount_MultiLine(int constrainedSideLength)
	{
		T* pT = static_cast<T*>(this);
		DWORD dwStyle = this->GetStyle();
		int constrainedSideLengthAdjusted = constrainedSideLength;
		if(CTCS_VERTICAL == (dwStyle & CTCS_VERTICAL))
		{
			constrainedSideLengthAdjusted -= (m_settings.iTabAreaPaddingBottom + m_settings.iTabAreaPaddingTop);
		}
		else
		{
			constrainedSideLengthAdjusted -= (m_settings.iTabAreaPaddingLeft + m_settings.iTabAreaPaddingRight);
		}
		...
		int farLeft = m_settings.iIndent;
		int left=farLeft, right=farLeft, itemWidth=0;
		int tabsOnCurrentRow=0;
		m_iRowCount = 1;
		for( size_t i=0; i<nCount; ++i )
		{
			pT->CalcSize_Item_FullSize(i, CDCHandle(dc), &itemWidth);
			right = left + itemWidth;

			if((right + m_settings.iIndent) < constrainedSideLengthAdjusted)
			{
				tabsOnCurrentRow += 1;
				left = right + m_settings.iMargin;
			}
			else
			{
				if(tabsOnCurrentRow == 0)
				{
					tabsOnCurrentRow = 0;
					if(i < (nCount-1))
					{
				...
				else
				{
					tabsOnCurrentRow = 1;
					m_iRowCount += 1;
					left = farLeft + itemWidth + m_settings.iMargin;
				}
			}
		}
	...
	void UpdateLayout_MultiLine(RECT rcTabItemArea)
	{
		// This depend on CalcRowCount_MultiLine already being called,
		// and our size being set appropriately.  We'll depend on the
		// caller to subtract off our padding.
		// 
		...
		// First, calculate the "natural" layout of the tabs
		pT->UpdateLayout_MultiLine_NaturalLayout(tabHeight, rcTabItemArea);

		if(m_iRowCount > 1)
		{
			pT->UpdateLayout_MultiLine_DistributeItemsAcrossRows(rcTabItemArea);

			// If there is more than one row, and the style says so,
			// grow the tab widths on each row so that the row is fully justified.
			if(CTCS_RAGGEDRIGHT != (dwStyle & CTCS_RAGGEDRIGHT))
			{
				pT->UpdateLayout_MultiLine_FullJustifyEachRow(rcTabItemArea);
			}

			pT->UpdateLayout_MultiLine_PrepareRowOrder();

			// Only apply the EnsureSelectedTabRow logic if not dragging.
			if(ectcDraggingItem != (m_dwState & ectcDraggingItem))
			{
				// If there are multiple rows, make sure the selected tab
				// is either at the bottom (if tabs are on top) or the top
				// (if tabs are on bottom - CTCS_BOTTOM)
				pT->UpdateLayout_MultiLine_EnsureSelectedItemRowOrder();
			}

			// Rearrange remaining rows in a "most-recently used" way
			// like the SysTabControl32 does.
			pT->UpdateLayout_MultiLine_ApplyRowOrder();
		}
	}

	void UpdateLayout_MultiLine_NaturalLayout(int tabHeight, RECT rcTabItemArea)
	{
		T* pT = static_cast<T*>(this);
		...
		//  interprets margin, padding, etc.
		size_t nCount = m_Items.GetCount();
		int farLeft = rcTabItemArea.left + m_settings.iIndent;
		int left=farLeft, right=farLeft, top=rcTabItemArea.top, bottom=top+(tabHeight-1), itemWidth=0;
		int tabsOnCurrentRow=0, currentRow = 0;
		for( size_t i=0; i<nCount; ++i )
		{
			TItem* pItem = m_Items[i];
			ATLASSERT(pItem != NULL);

			pItem->ClearContentOffset();

			pT->CalcSize_Item_FullSize(i, CDCHandle(dc), &itemWidth);

			right = left + itemWidth;

			if((right + m_settings.iIndent) < rcTabItemArea.right)
			{
				pItem->SetRect(left, top, right, bottom);
	...
	void UpdateLayout_MultiLine_DistributeItemsAcrossRows(RECT rcTabItemArea)
	{
		// The system tab control goes to greater lengths to distribute tab
		// items across rows.  We'll make an effort, but take advantage
		// of the fact that the rows up until the last one are packed as tight
		// as they can be.  So, we'll start from the bottom row, and "pull down"
		// items as we can to try to get the ideal tabs-per-row
		// (for now, emphasizing item count over item size - a better solution
		// would be to consider the size of each item, and try to equalize the
		// amount of "left over" space for each row).
		T* pT = static_cast<T*>(this);

		size_t nCount = m_Items.GetCount();
		if(nCount > 1)
		{
			size_t idealTabsPerRow = (size_t)(((nCount * 1.0f) / (m_iRowCount * 1.0f)) + 0.5f);

			// Start from the bottom row, and try to have the ideal number of tabs per row
			int currentTop=0, lastTop = -1;
			int rowIndexFirst = -1, rowIndexLast = -1;
			int rowAboveIndexFirst = -1, rowAboveIndexLast = -1;
			...
			for(int i=nCount-1; i>=0; --i)
			{
				...
				currentTop = pItem->GetRectRef()->top;

				if(currentTop != lastTop)
				{
					if(rowIndexLast == -1)
					{
						ATLASSERT(i == (int)(nCount - 1));
						rowIndexLast = i;
					}
					else if(rowIndexFirst == -1)
					{
						rowIndexFirst = i+1;
						rowAboveIndexLast = i;
					}
					else if(rowAboveIndexFirst == -1)
					{
						rowAboveIndexFirst = i+1;

						// OK, we know the items on the 2 rows.
						// Try to pull items from the "above" row
						// into the target row until it has the ideal
						// number of tabs, or until more tabs wouldn't fit
						int newRowIndexFirst=0;
						pT->UpdateLayout_MultiLine_DistributeItemsBetweenTwoRows(rcTabItemArea,
							idealTabsPerRow,
							rowIndexFirst, rowIndexLast,
							rowAboveIndexFirst, rowAboveIndexLast,
							&newRowIndexFirst);

						// Reset variables so that the "above" row is now the target row,
						// and we're searching for the next above row.
						rowIndexLast = newRowIndexFirst-1;
						rowIndexFirst = rowAboveIndexFirst;
						rowAboveIndexLast = i;
						rowAboveIndexFirst = -1;
					}
				}
			...
			if(rowAboveIndexFirst == -1 && rowAboveIndexLast >= 0)
			{
				rowAboveIndexFirst = 0;
				int newRowIndexFirst=0;
				pT->UpdateLayout_MultiLine_DistributeItemsBetweenTwoRows(rcTabItemArea,
					idealTabsPerRow,
					rowIndexFirst, rowIndexLast,
	...
	void UpdateLayout_MultiLine_DistributeItemsBetweenTwoRows(RECT rcTabItemArea,
				size_t idealTabsPerRow,
				int rowIndexFirst, int rowIndexLast,
				int rowAboveIndexFirst, int rowAboveIndexLast,
				int* newRowIndexFirst)
	{
		TItem* pItemFirst = m_Items[rowIndexFirst];
		TItem* pItemLast = m_Items[rowIndexLast];

		*newRowIndexFirst = rowIndexFirst;

		DWORD dwStyle = this->GetStyle();
		if(CTCS_VERTICAL == (dwStyle & CTCS_VERTICAL))
		{
			//int rowWidth = pItemFirst->GetRectRef()->bottom - pItemLast->GetRectRef()->top;
			//int availableSpace = ((rcTabItemArea.bottom - rcTabItemArea.top) - (2*m_settings.iIndent)) - rowWidth;
		}
		else
		{
			int rowWidth = pItemLast->GetRectRef()->right - pItemFirst->GetRectRef()->left;
			int availableSpace = ((rcTabItemArea.right - rcTabItemArea.left) - (2*m_settings.iIndent)) - rowWidth;

			int itemsToMove = 0;
			int itemsAlreadyOnTargetRow = (rowIndexLast - rowIndexFirst) + 1;
			int itemsOnAboveRow = (rowAboveIndexLast - rowAboveIndexFirst) + 1;

			if(rowAboveIndexLast > rowAboveIndexFirst)
			{
				// There's at least 2 items in the "above" row
				int left=rcTabItemArea.left + m_settings.iIndent,top=0,right=left,bottom=0;
				top = pItemFirst->GetRectRef()->top;
				bottom = pItemFirst->GetRectRef()->bottom;

				for(int i=rowAboveIndexLast; i>=rowAboveIndexFirst; --i)
				{
					TItem* pItem = m_Items[i];
					LPCRECT rect = pItem->GetRectRef();
					if( i == rowAboveIndexFirst ||
						(itemsToMove + itemsAlreadyOnTargetRow) >= (int)idealTabsPerRow ||
						(itemsOnAboveRow - itemsToMove) <= (itemsToMove + itemsAlreadyOnTargetRow) ||
						availableSpace < (rect->right - rect->left))
					{
						// pull down the items from i+1 to rowIndexLast
						// to the row underneath
						*newRowIndexFirst = i+1;
						for(int j=i+1; j<=rowIndexLast; ++j)
						{
							TItem* pItemToAdjust = m_Items[j];
							RECT rcItem = pItemToAdjust->GetRect();

							right = left + (rcItem.right - rcItem.left);

							rcItem.top = top;
							rcItem.bottom = bottom;
							rcItem.left = left;
							rcItem.right = right;
							pItemToAdjust->SetRect(rcItem);
	...
	void UpdateLayout_MultiLine_FullJustifyEachRow(RECT rcTabItemArea)
	{
		T* pT = static_cast<T*>(this);

		int currentTop=0, lastTop = -1;
		int rowIndexFirst = 0, rowIndexLast = 0;
		...
		if(nCount > 0)
		{
			for( size_t i=0; i<nCount; ++i )
			{
				...
				if(currentTop != lastTop)
				{
					rowIndexLast = ((int)i)-1;
					// Process new row
					pT->UpdateLayout_MultiLine_FullJustifyRow(
						rcTabItemArea, rowIndexFirst, rowIndexLast);
	...
	void UpdateLayout_MultiLine_FullJustifyRow(RECT rcTabItemArea, int rowIndexFirst, int rowIndexLast)
	{
		if((rowIndexLast >= rowIndexFirst) && (rowIndexFirst >= 0) && (rowIndexLast < (int)m_Items.GetCount()))
		{
			int left=rcTabItemArea.left + m_settings.iIndent, right=left, width=0;
			short contentOffsetLeft = 0, contentOffsetRight = 0;

			// Distribute the extra space at the end as equally as possible amongst the tabs.
			int extraSpace = rcTabItemArea.right - (m_Items[rowIndexLast]->GetRectRef()->right + m_settings.iIndent);
			int spaceToAddTruncated = extraSpace / (rowIndexLast-rowIndexFirst + 1);
			int itemsToGivePlusOne = extraSpace % (rowIndexLast-rowIndexFirst + 1);

			int tabPositionOnRow = 1;
			for(int i=rowIndexFirst; i<=rowIndexLast; ++i)
			{
				...
				right = left + width + spaceToAddTruncated;
				if(tabPositionOnRow <= itemsToGivePlusOne)
				{
					right += 1;
				}
				...
				// NOTE: Content offset (left and right) is used to center the image and text.
				//  Content offset left and right will cause the "content rect" to go
				//  between being exactly right and 1 pixel too big on _one_ side.  Be sure they
				//  never cause the content rect to go too small (otherwise, we'd get ellipsis
				//  on text when we don't want)
				contentOffsetLeft = (short)((((right - left) - width) / 2) + (((right - left) - width) % 2));
				contentOffsetRight = (short)(((right - left) - width) / 2);
				pItem->SetContentOffset(contentOffsetLeft, contentOffsetRight);
	...
	void UpdateLayout_MultiLine_PrepareRowOrder()
	{
		if((int)m_RowOrder.GetCount() != m_iRowCount)
		{
			// The row order has been reset.
			// Reset the order to be ready for a change.
			m_RowOrder.RemoveAll();
			for(short i=0; i<m_iRowCount; ++i)
			{
				m_RowOrder.Add(i);
			}
		}
	...
	void UpdateLayout_MultiLine_EnsureSelectedItemRowOrder()
	{
		if(m_iCurSel < 0 || m_iRowCount <= 1)
		{
			// No tab is selected, or there is only one row
			return;
		}
		size_t nCount = m_Items.GetCount();
		if(nCount <= 1 || m_iCurSel >= (int)nCount)
		{
			// The selected tab index doesn't reference a current tab,
			// or there is only one item.
			return;
		}
		int rowOrderCount = (int)m_RowOrder.GetCount();
		ATLASSERT(rowOrderCount == m_iRowCount);

		// Determine the row of the selected item after all previous processing of the natural order
		// and distributing items across rows.
		int currentTop=0, lastTop = -1;
		short currentRow = 0, selectedItemRow = -1;

		int selectedTop = m_Items[m_iCurSel]->GetRectRef()->top;

		for( size_t i=0; i<nCount && selectedItemRow<0; ++i )
		{
			TItem* pItem = m_Items[i];
			ATLASSERT(pItem != NULL);
			RECT rcItem = pItem->GetRect();

			currentTop = rcItem.top;
			...
			{
				selectedItemRow = currentRow;
			}

			lastTop = currentTop;
		}

		// Now that we know the row, shift it to the desired location,
		// shifting the items between it and the desired location.

		ATLASSERT(selectedItemRow >= 0 && selectedItemRow < rowOrderCount);
		if(selectedItemRow >= 0 && selectedItemRow < rowOrderCount)
		{
			short desiredRowForSelectedItem = 0;
			bool shiftDown = true;

			DWORD dwStyle = this->GetStyle();
			if(CTCS_BOTTOM == (dwStyle & CTCS_BOTTOM))
			{
				// Make sure the selected tab is on the top row
				desiredRowForSelectedItem = 0;
				shiftDown = true;
			}
			else
			{
				// Make sure the selected tab is on the bottom row
				desiredRowForSelectedItem = (short)(m_iRowCount-1);
				shiftDown = false;
			}

			if(m_RowOrder[selectedItemRow] != desiredRowForSelectedItem)
			{
				// We have a new selected item it seems.  We want to mimic the SysTabCtrl32 behavior of having
				// a "most recently used" algorithm for displaying the row order.  So, we'll
				// "push this row on the stack" (bottom or top - depending on the tab style).

				short currentSelectedItemRowOrder = m_RowOrder[selectedItemRow];
				for(short row=0; row<rowOrderCount; ++row)
				{
					short rowOrder = m_RowOrder[row];
					if(row != selectedItemRow)
					{
						if(shiftDown)
						{
							if(rowOrder < currentSelectedItemRowOrder)
							{
								m_RowOrder[row] = (short)(rowOrder + 1);
							}
						}
						else
						{
				...
				m_RowOrder[selectedItemRow] = desiredRowForSelectedItem;
			}
		}
	}

	void UpdateLayout_MultiLine_ApplyRowOrder()
	{
		int tabHeight = m_settings.iTabDefaultHeight;
		if(m_settings.iTabForcedHeight > 0)
		{
		...
		int currentTop=0, lastTop = -1, firstTop = -1;
		short currentRow = 0;
		...
		if(nCount > 0)
		{
			for( size_t i=0; i<nCount; ++i )
			{
				...
				rcItem.top = firstTop + (m_RowOrder[currentRow] * tabHeight);
				rcItem.bottom = rcItem.top + (tabHeight - 1);
	...
	void UpdateLayout_ScrollToFit(RECT rcTabItemArea)
	{
		...
		//  interprets margin, padding, etc.
		size_t nCount = m_Items.GetCount();
		int farLeft = rcClient.left + m_settings.iIndent;
		int left=farLeft, right=farLeft, top=rcTabItemArea.top, bottom=top+(tabHeight-1), itemWidth=0;
		for( size_t i=0; i<nCount; ++i )
		{
			TItem* pItem = m_Items[i];
		...
		if((right + m_settings.iIndent + m_iScrollOffset) < rcTabItemArea.right)
		{
			m_iScrollOffset = (rcTabItemArea.right - (right + m_settings.iIndent));
		}
...
public:
	...
	*/
	...
	DWORD GetItemSize(int cx, int cy) const
	{
		return MAKELONG(m_settings.iTabForcedWidth, m_settings.iTabForcedHeight);
	}

	DWORD SetItemSize(int cx, int cy)
	{
		int oldForcedWidth = m_settings.iTabForcedWidth;
		int oldForcedHeight = m_settings.iTabForcedHeight;
		m_settings.iTabForcedWidth = cx;
		m_settings.iTabForcedHeight = cy;
		return MAKELONG(oldForcedWidth, oldForcedHeight);
	}

	// TODO: Review if we need this.  If someone changes the size of an individual tab item,
	//  we really would need to resize everything and remember the size of the item through
	//  any automatic resizing.  For now, we'll have SetItemSize be like the SysTabCtrl32
	//  and serve as the "force item size" function for all tabs.
	...
	{
		return m_settings.iTabMinWidth;
	}

	int SetMinTabWidth(int cx)
	{
		int oldMinTabWidth = m_settings.iTabMinWidth;
		m_settings.iTabMinWidth = cx;
		return oldMinTabWidth;
	}
