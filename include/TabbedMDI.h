...
// - Tab controls are now responsible for their size.  Obsolete "tab area height"
//   related member variables and methods (such as SetTabAreaHeight and
//   CalcTabAreaHeight)
// - Handle the new CTCN_NEEDRESIZE notification when the tab control needs
//   to be resized (its calculated bounds doesn't agree with its client rect).
//   Trigger a resize / WM_POSCHANGING for the MDI client.
// - Change CTabbedMDIClient::OnWindowPosChanging to use new CalculateBounds.
// - Have call to CMDITabOwnerImpl::CreateTabWindow be overrideable.
//
...
public:
	...
	BEGIN_MSG_MAP(thisClass)
		...
		NOTIFY_CODE_HANDLER(CTCN_NEEDRESIZE, OnNeedResize)
	...
	LRESULT OnCreate(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
	{
		...
		T* pT = static_cast<T*>(this);
		pT->CreateTabWindow(m_hWnd, rcDefault, m_nTabStyles);
	...
	LRESULT OnSize(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& bHandled)
	{
		...
		// The SetWindowPos will cause a WM_SIZE, which will cause an UpdateLayout
	...
	LRESULT OnNeedResize(int /*idCtrl*/, LPNMHDR pnmh, BOOL& bHandled)
	{
		// Be sure the notification is from the tab control
		// (and not from a sibling like a list view control)
		if(pnmh && (m_TabCtrl == pnmh->hwndFrom))
		{
			NMCTCSIZE* resizeRequest = (NMCTCSIZE*)pnmh;
			if(resizeRequest != NULL)
			{
				RECT rcTabControl={0};
				m_TabCtrl.GetWindowRect(&rcTabControl);

				int nOldTabAreaHeight = rcTabControl.bottom - rcTabControl.top;
				int nNewTabAreaHeight = resizeRequest->rect.bottom - resizeRequest->rect.top;

				//T* pT = static_cast<T*>(this);
				//pT->UpdateLayout();
				//Invalidate();

				if(nOldTabAreaHeight != nNewTabAreaHeight && this->IsWindowVisible() == TRUE)
				{
					RECT rcMDIClient;
					::GetWindowRect(m_hWndMDIClient, &rcMDIClient);
	...
	LRESULT OnWindowPosChanging(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
	{
		...
		if(pWinPos)
		{
			if( m_MdiTabOwner.IsWindow() )
			{
				...
				RECT rect={pWinPos->x, pWinPos->y,
					pWinPos->x + pWinPos->cx, pWinPos->y + pWinPos->cy};
				RECT tabRect={0};
				RECT viewRect={0};
				if(m_MdiTabOwner.IsWindowVisible() == FALSE)
				{
					::SetRect(&tabRect, rect.left, rect.top, rect.right, rect.top);
					::CopyRect(&viewRect, &rect);
				}
				else
				{
					// NEW: m_MdiTabOwner.GetTabControl().CalculateBounds is now called before we commit a window
					//  size change, so that it can calculate how big it needs to be,
					//  and then we can use that size.
					//  (esp. if its setup for multiline).
					m_MdiTabOwner.GetTabCtrl().CalculateBounds(rect, &tabRect, &viewRect);
				}

				m_MdiTabOwner.SetWindowPos(NULL, &tabRect,
				...
				if(CTCS_BOTTOM == (dwStyle & CTCS_BOTTOM))
				{
					...
					{
						pWinPos->cy -= (tabRect.bottom - tabRect.top);
					}
				}
				else
				{
					...
					{
						pWinPos->y += (tabRect.bottom - tabRect.top);
					}
					if((pWinPos->flags & SWP_NOSIZE) == 0)
					{
						pWinPos->cy -= (tabRect.bottom - tabRect.top);
