...
//  
//
// Written by Daniel Bowen (bowen.daniel@gmail.com).
// Copyright (c) 2001-2007 Daniel Bowen.
//
...
//   (this was a sponsored enhancement) (see comment in CustomTabCtrl.h for more info)
// - Update drawing for new layout method.
// - Set defaults for new settings so that tabs draw like they did before.
// - Override the new CalcSize_Item_FullSize, CalcRowCount_MultiLine,
//   UpdateLayout_MultiLine_NaturalLayout, UpdateLayout_MultiLine_FullJustifyRow
// - Refactor other drawing styles (scroll, default) to use new CalcSize_Item_FullSize
//   as well as tab height, and so on.
// - Change DrawItem_InitBounds arguments (CDotNetTabCtrlImpl) to add custom
//   draw structure and "content" RECT.
//
// 2005/09/10: Daniel Bowen
// - DrawItem_ImageAndText now uses DT_END_ELLIPSIS instead of DT_PATH_ELLIPSIS,
//   so that if the text doesn't fit in the given space, ... is used at the end
//   to let you know there's more text.
// 
...
public:
	...
	LRESULT OnSettingChange(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& /*bHandled*/)
	{
		...
		const int nNominalTabHeight = 19;
		...
		m_settings.iTabDefaultHeight = (signed char)(nNominalTabHeight + (nHeightLogicalUnits - nNominalFontLogicalUnits));
		m_settings.iTabForcedHeight = 0;
		m_settings.iTabForcedWidth = 0;
		m_settings.iTabMinWidth = 0;

		// Tab area related settings
		if(CTCS_BOTTOM == (dwStyle & CTCS_BOTTOM))
		{
			//m_settings.iTabAreaPaddingLeft = 0;
			//m_settings.iTabAreaPaddingRight = 0;
			//m_settings.iTabAreaPaddingTop = 3;
			//m_settings.iTabAreaPaddingBottom = 0;
			m_settings.iTabAreaPaddingLeft = 0;
			m_settings.iTabAreaPaddingRight = 0;
			m_settings.iTabAreaPaddingTop = 3;
			m_settings.iTabAreaPaddingBottom = 2;
		}
		else
		{
			//m_settings.iTabAreaPaddingLeft = 2;
			//m_settings.iTabAreaPaddingRight = 2;
			//m_settings.iTabAreaPaddingTop = 1;
			//m_settings.iTabAreaPaddingBottom = 2;
			m_settings.iTabAreaPaddingLeft = 2;
			m_settings.iTabAreaPaddingRight = 2;
			m_settings.iTabAreaPaddingTop = 4;
			m_settings.iTabAreaPaddingBottom = 1;
		}
...
public:
	...
	void DrawItem_InitBounds(DWORD dwStyle, LPNMCTCCUSTOMDRAW lpNMCustomDraw, RECT rcItem, RECT& /*rcTab*/, RECT& rcContent, int& nIconVerticalCenter)
	{
		...
		rcContent.top = rcItem.top+1 + m_nFontSizeTextTopOffset;
		rcContent.bottom = rcItem.bottom;
		rcContent.left = rcItem.left + pItem->GetContentOffsetLeft();
		rcContent.right = rcItem.right + pItem->GetContentOffsetRight();

		nIconVerticalCenter = (rcItem.bottom + rcItem.top) / 2;
		if(CTCS_BOTTOM == (dwStyle & CTCS_BOTTOM))
		{
			nIconVerticalCenter += 1;
		}
	}
	...
	void DrawItem_TabInactive(DWORD dwStyle, LPNMCTCCUSTOMDRAW lpNMCustomDraw, RECT& rcTab)
	{
		...
		bool drawRightSideDivider = true;

		if(nItem + 1 == m_iCurSel && m_iCurSel < (int)m_Items.GetCount())
		{
			// Item just left of selected tab
			drawRightSideDivider = false;
			if(CTCS_MULTILINE == (dwStyle & CTCS_MULTILINE))
			{
				if(rcTab.right > m_Items[m_iCurSel]->GetRectRef()->left)
				{
					// The tab is the last one on its row, and the selected
					// tab is the first one on the next row.  Draw the divider.
					drawRightSideDivider = true;
				}
			}
		}

		if(drawRightSideDivider)
		{
			WTL::CPen pen;
	...
	void DrawItem_ImageAndText(DWORD /*dwStyle*/, LPNMCTCCUSTOMDRAW lpNMCustomDraw, int nIconVerticalCenter, RECT& rcTab, RECT& rcContent)
	{
		WTL::CDCHandle dc( lpNMCustomDraw->nmcd.hdc );
		bool bHighlighted = (CDIS_MARKED == (lpNMCustomDraw->nmcd.uItemState & CDIS_MARKED));
		bool bSelected = (CDIS_SELECTED == (lpNMCustomDraw->nmcd.uItemState & CDIS_SELECTED));
		...
		//rcContent.left += (bSelected ? m_settings.iSelMargin : m_settings.iMargin);
		rcContent.left += m_settings.iMargin;
		rcContent.right -= m_settings.iMargin;
		if (pItem->UsingImage() && !m_imageList.IsNull())
		{
			...
			if((ii.rcImage.right - ii.rcImage.left) < (rcTab.right - rcTab.left))
			{
				...
				m_imageList.Draw(dc, nImageIndex, rcContent.left, nIconVerticalCenter - nImageHalfHeight + m_nFontSizeTextTopOffset, ILD_NORMAL);
			}

			// Offset on the right of the image.
			rcContent.left += (ii.rcImage.right - ii.rcImage.left);
		}

		if (rcContent.left + m_nMinWidthToDisplayText < rcContent.right)
		{
			::InflateRect(&rcContent, -m_settings.iPadding, 0);

			_CSTRING_NS::CString sText = pItem->GetText();
			dc.DrawText(sText, sText.GetLength(), &rcContent, DT_LEFT | DT_VCENTER | DT_SINGLELINE | DT_NOPREFIX | DT_END_ELLIPSIS);
		}
...
public:
	...
	void DoItemPaint(LPNMCTCCUSTOMDRAW lpNMCustomDraw)
	{
		...
		pT->DrawItem_InitBounds(dwStyle, lpNMCustomDraw, rcItem, rcTab, rcContent, nIconVerticalCenter);
		...
		pT->DrawItem_ImageAndText(dwStyle, lpNMCustomDraw, nIconVerticalCenter, rcTab, rcContent);
	}
...
public:
	...
	// The equivalent of CalcSize_NonClient is done in the base
	// UpdateLayout, and uses the iTabAreaPadding* settings from
	// m_settings.
	...
	void CalcSize_CloseButton(LPRECT prcTabItemArea)
	{
		...
		::CopyRect(&m_rcCloseButton, prcTabItemArea);
	...
	void CalcSize_ScrollButtons(LPRECT prcTabItemArea)
	{
		...
		RECT rcScroll={0};
		::CopyRect(&rcScroll, prcTabItemArea);
	...
	void UpdateLayout_Default(RECT rcTabItemArea)
	{
		...
		for( size_t i=0; i<nCount; ++i )
		{
			...
			if((int)i != m_iCurSel)
			{
				if(itemWidth < nMinInactiveWidth)
				{
					nMinInactiveWidth = itemWidth;
				}
				if(itemWidth > nMaxInactiveWidth)
				{
					nMaxInactiveWidth = itemWidth;
				}
			}
		...
		if((right + m_settings.iIndent) > rcTabItemArea.right && nCount > 0 && m_iCurSel >= 0)
		{
			...
			LPCRECT prcSelected = m_Items[m_iCurSel]->GetRectRef();
			LONG nSelectedWidth = (prcSelected->right - prcSelected->left);

			long cxClientInactiveTabs = nTabAreaWidth - (m_settings.iIndent * 2) - nSelectedWidth;
			long cxDesiredInactiveTabs = right - m_settings.iIndent - nSelectedWidth;
			...
			if(cxClientInactiveTabs > (nInactiveSameSizeWidth * (long)(nCount-1)))
			{
				...
				int xpos = rcTabItemArea.left + m_settings.iIndent;
				RECT rcItem = {xpos, top, xpos, bottom};
				for(size_t i=0; i<nCount; ++i )
				{
					TItem* pItem = m_Items[i];
					ATLASSERT(pItem != NULL);
