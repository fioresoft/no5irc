...
public:
	...
	{
		CString sCommaList;
		sCommaList = MakeCommaList<TCHAR>(lst);
		...
		DWORD nSize)
	{
		...
		ATLASSERT( !m_path.IsEmpty() );
		{
			CStringBuffer<TCHAR> buf(value,nSize);
	...
	{
		CString sDefault = MakeCommaList<TCHAR>(lst);
		CString res = GetString(section,key,sDefault,1024);
		lst.RemoveAll();
		return MakeStringList<TCHAR>(res,lst);
	}
	int GetIntList(LPCTSTR section,LPCTSTR key,CSimpleValArray<int> &lst)
	{
		CString sDefault = NO5TL::MakeCommaList<TCHAR>(lst);
		CString res = GetString(section,key,sDefault,1024);
		lst.RemoveAll();
		return NO5TL::MakeIntList<TCHAR>(res,lst);
	}
	*/
