	...
	void UpdateSplitterLayout()
	{
		...
		if(m_nSinglePane == SPLIT_PANE_NONE)
		{
			...
			for(int nPane = 0; nPane < m_nPanesCount; nPane++)
			{
				if(GetSplitterPaneRect(nPane, &rect))
				{
					if(m_hWndPane[nPane] != NULL){
						/* BEGIN_CHANGE */
						CWindow wndParent = ::GetParent(m_hWndPane[nPane]);

						if(pT->m_hWnd != wndParent.m_hWnd){
							pT->MapWindowPoints(wndParent,&rect);
						}
						BOOL bDebug = ::SetWindowPos(m_hWndPane[nPane], HWND_TOP, rect.left, rect.top, rect.right - rect.left, rect.bottom - rect.top, 0);
						ATLASSERT(bDebug);
						/* END_CHANGE */
					}
					else
		...
		else
		{
			if(GetSplitterPaneRect(m_nSinglePane, &rect))
			{
				if(m_hWndPane[m_nSinglePane] != NULL){
						/* BEGIN_CHANGE */
						CWindow wndParent = ::GetParent(m_hWndPane[m_nSinglePane]);

						if(pT->m_hWnd != wndParent.m_hWnd){
							pT->MapWindowPoints(wndParent,&rect);
						}
						BOOL bDebug = ::SetWindowPos(m_hWndPane[m_nSinglePane], HWND_TOP, rect.left, rect.top, rect.right - rect.left, rect.bottom - rect.top, 0);
						ATLASSERT(bDebug);
						/* END_CHANGE */
				}
				else
