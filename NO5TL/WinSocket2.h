...
// CWinSocket, making it easier to use
class CWinSocket2 : public CWinSocket,public CAsyncBase
{
public: /** TODO: nao encontrei CAsyncSocket substituí por CWinSocket */
	CWinSocket2(ISocketEvents *pSink):CWinSocket(pSink)
	{
	...
	{
		BOOL res = CWinSocket::CreateSocket(type,proto);
	...
	virtual BOOL AttachAcceptedSocket(SOCKET s)
	{
		...
		BOOL res = CAsyncSocketBase::AttachAcceptedSocket(s);
...
public:
	...
	virtual BOOL GetHostByName(LPCTSTR name)
	{
		...
		BOOL res;
		SetHandler(GetHandler().m_hWnd);
		res = CAsyncAddress::GetHostByName(T2CA(name));
		if(res){
